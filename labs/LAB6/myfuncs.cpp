...
void cntD(MLL *mll, Dialog::Ioeqpp &iqp) {
  if (iqp.printQst)
    iqp.qout << "Write string, which ends by \'$\' sign:" << std::endl;
  std::string str;
  getline(iqp.in, str, '$');
  iqp.in.ignore();
  std::vector<Dialog::FuncWithDesc<uint64_t(MLL *, std::string &)>> options = {
      {"same full name",
       [](MLL *mll, std::string &str) {
         return MLL_cnt_coincidences(*mll, str, 0);
       }},
      {"same job",
       [](MLL *mll, std::string &str) {
         return MLL_cnt_coincidences(*mll, str, 1);
       }},
      {"same place",
       [](MLL *mll, std::string &str) {
         return MLL_cnt_coincidences(*mll, str, 2);
       }},
  };
  iqp.out
      << options[Dialog::optionD<uint64_t(MLL *, std::string &)>(options, iqp)]
             .func(mll, str)
      << std::endl;
}
} // namespace
