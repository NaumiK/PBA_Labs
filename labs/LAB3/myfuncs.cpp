...
Matrix iXY(Dialog::Ioeqpp &iqp) {
  try {
    if (iqp.printQst)
      iqp.qout << "n" << std::endl;
    size_t n;
    if (MIO::inputULL(n, iqp.in, iqp.eout, iqp.printErr) != 0)
      throw MExc::ThereIsNothing();
    Matrix mrx{n, n};
    if (iqp.printQst)
      iqp.qout << "Matrix nxn" << std::endl;
...
void oXY(const std::vector<int64_t> &vr, const std::string &exc,
         std::ostream &out) {
  out << exc << ") ";
...
void MDialog::ioXY(Dialog::Ioeqpp &iqp) {
  ...
  for (size_t i = 0, n = ans.size(); i < n; ++i) {
    ...
    for (size_t j = 1; j < n; ++j) {
      ans[i] =
          ans[i] && ((mrx[i][0] < mrx[i][n - 1] && mrx[i][j - 1] < mrx[i][j]) ||
...
void oXY(const uint32_t *inp, size_t n, const std::string &exs,
         std::ostream &out) {
  out << exs << ") ";
  for (int i = 1; i <= n; ++i) {
    bool f = false;
...
void MDialog::ioXY(Ioeqpp &iqp) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ans[i] = 1;
    for (size_t j = 1; j < n; ++j) {
      ans[i] = ans[i] && ((mrx[i * n] < mrx[(i + 1) * n - 1] &&
                           mrx[i * n + j - 1] < mrx[i * n + j]) ||
                          (mrx[i * n] > mrx[(i + 1) * n - 1] &&
                           mrx[i * n + j - 1] > mrx[i * n + j]));
    }
  }
