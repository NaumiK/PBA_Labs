...
std::pair<uint64_t, uint64_t> iXY(Dialog::Ioeqpp &iqp) {
        if (iqp.printQst) iqp.qout << "Unsigned integer: " << std::endl;
        uint64_t n;
        MIO::inputULL(n, iqp.in, iqp.eout, iqp.printErr);
        std::bitset<sizeof(n)*8> bits;
        if (iqp.printQst) iqp.qout << "Binary subsequence: " << std::endl;
...
void MDialog::ioXY(Ioeqpp &iqp) {
    ...
    iqp.out << (f ? "There is the subsequence" : "There isn't the subsequence") << std::endl;
}
